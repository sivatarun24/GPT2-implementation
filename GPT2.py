...
class CasualSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.register_buffer("bias", torch.tril(torch.ones(config.n_block, config.n_block)).view(1, 1, config.n_block, config.n_block))
    ...
    def forward(self, x):
        ...
        att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size - 1)) # calculate attn
        att = att.masked_fill(self.bias[:,:,:T,:T] == 0, float('inf')) # mask attn
...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.c_fn = nn.LayerNorm(config.n_embd, 4 * config.n_embd)
        self.gelu = nn.Gelu(approximate="tanh")
        self.c_proj = nn.LayerNorm(4 * config.n_embd, config.n_embd)
...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        sd_keys = sd.keys()
        sd_keys = [k for k in sd_keys if not k.endswith('.attn.bias')] # we are removing bias
        ...
        sd_hf_keys = [k for k in sd_hf_keys if not k.endswith('.attn.bias')] # we are removing bias

        # set weights from hugging face to our model
        transposed = ['attn.c_attn.weight', 'attn.c_proj.weight', 'mlp.c_fn.weight', 'mlp.c_proj.weight']
